...
return t}
function btExtractText(str){var ret="";var skip=2;if(jQuery.isArray(str)){matchResult=str;skip=1}
else{if('string'!==typeof(str)){str=str.toString()}
if(/"/.test(str)){matchResult=str.match(/"(.*?)"/g)}else{matchResult=str.split(",");skip=5}}
if(matchResult.length<3){ret+=matchResult[0]}else{for(i=0;i<matchResult.length-3;i=i+skip){sResult=matchResult[i].replace(/(^")|("$)/g,'').trim();if((sResult==='\n')||(sResult==='\r')){}else{ret+=sResult+'<br />'}}}
return ret}
...
if(!sourceLang){sourceLang='auto'}
if(!targetLang){targetLang=document.documentElement.lang}
startSpinner();var url="//translate.googleapis.com/translate_a/single?client=gtx&sl="+sourceLang+"&tl="+targetLang+"&dt=t&q="+encodeURI(q);var href="http://translate.google.cn/#"+mapGoogleLanguageCode(sourceLang)+'/'+mapGoogleLanguageCode(targetLang)+'/'+encodeURI(q);jQuery('#bTranslateLink').attr('href',href);var msBeforeAjaxCall=new Date().getTime();jQuery.ajax(url,{"type":"GET","timeout":9000}).done(function(data,textStatus,jqXHR){translateResult='';if(jQuery.isArray(data)){data=data[0]}
if(jQuery.isArray(data)){for(i=0;i<data.length;i=i+1){sResult=data[i];if(jQuery.isArray(sResult)){sResult=sResult[0]}
sResult.replace(/(^")|("$)/g,'').trim();if((sResult==='\n')||(sResult==='\r')){}else{translateResult+=sResult+'<br />'}}}else{translateResult=btExtractText(data)}
var element=jQuery('#bTranslateResult');element.text(translateResult);element.html(element.text().replace(/\\n\\n/g,'<br />').replace(/\\n/g,'<br />'));element=jQuery('#bTranslateContainer').detach();parentEl.append(element);element.css("display","block")}).fail(function(jqXHR,textStatus,errorThrown){debugger
var translateResult='';switch(textStatus){case "parsererror":break;case "timeout":translateResult='Translation timed out.\n';break;default:translateResult=' Failed on: '+textStatus+'\n'}
